      ...
      >
        ...
        {isUploading && (
          <div className="absolute inset-0 flex items-center justify-center bg-black/50">
            <Loader data-testid="input-attachment-loader" size={16} />
          </div>
        )}
      ...
      >
        <DialogTrigger asChild>
          <button
            ...
            className="size-full cursor-pointer outline-none ring-primary transition focus-visible:ring-2"
            data-testid="input-attachment-preview"
            onClick={(event) => {
              if (isUploading) {
        ...
        {showDownload && !isUploading ? (
          <a
            aria-label={name ? `Download ${name}` : "Download image attachment"}
