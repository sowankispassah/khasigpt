...
export function exceedsReferenceImageSizeLimit(buffer: ArrayBuffer) {
  return buffer.byteLength > MAX_IMAGE_UPLOAD_BYTES;
}
...
export async function buildCharacterReference({
  ...
  const characterMatches = await detectCharacters({
    prompt,
    aliasIndex,
    getCharactersByIds: deps?.getCharactersByIds,
  });
  ...
  const fetchCharacter = deps?.getCharacterById;
  if (!fetchCharacter) {
    return {
      prompt,
  ...
  const fetcher = deps?.fetchReferenceImage;
  if (!fetcher) {
    return {
