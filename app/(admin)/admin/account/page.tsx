...
function renderCostFeatureCard(
  row: Awaited<ReturnType<typeof getAdminApiCostBreakdown>>["featureSummaries"][number],
  currency: CostCurrency,
  usdToInr: number
) {
...
function renderChatProfitCard(row: ChatProfitRow) {
  const dateLabel = row.createdAt ? format(row.createdAt, "PPpp") : "-";
  return (
...
function renderRechargeCard(row: RechargeTableRow) {
  return (
    <MobileCard
...
function renderModelPricingCard(row: ModelPricingRow, usdToInr: number) {
  const totalProviderInr = row.totalProviderUsd * usdToInr;
  return (
