...
async function enforceLiveTranslationTokenRateLimit(
  ...
  const { allowed, resetAt } = await incrementRateLimit(
    `live-translation-token:web:${userId}:${clientKey}`,
...
export async function POST(request: Request) {
  ...
  const [accessMode, liveVoiceModel, settings] = await Promise.all([
    withApiTiming(
      "web.live-translation-token.settings",
      () => getLiveTranslationAccessModeForPlatform("web"),
      { slowMs: 750 }
    ).catch((error) => {
      console.error(
    ...
    withApiTiming(
      ...
      console.error(
        "[api/live-translation/token] Model config read failed.",
        error
      );
      return null;
    }),
  ...
  if (!(await isFeatureEnabledForUser({
    ...
    source: "api.live-translation.token.user-feature-access",
  ...
  const hasCredits = await withApiTiming(
    ...
    console.error("[api/live-translation/token] Credit read failed.", error);
    return false;
  });
  ...
  const token = await withApiTiming(
    "web.live-translation-token.google-token",
    () =>
      withTimeout(
    ...
    console.error("[api/live-translation/token] Token creation failed.", error);
    return null;
  });
